...
def recommend():
    user_id = session.get("user_id")
    fav_id = db.execute("SELECT id FROM lists WHERE user_id = ? and list_title = ?", user_id, "Favourites")[0]["id"]
    favs = getMovieinList(fav_id)
    recs = []
    for movie in favs:
        rec = dict()
        title = findMovies.getMovieDetails(str(movie))["title"]
        rec_movies = findMovies.getRecommend(movie)
        rec["title"] = title
        rec["movies"] = rec_movies
        recs.append(rec)
